...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($result->num_rows > 0) {
        ...
        while ($row = $result->fetch_assoc()) {
            ...
            echo "<td id='acc_fees'><select name='acc_fees[]' id='acc_fees" . $row['acc_fees'] . "'>";

            // Display the fetched value from the database
           // echo "<option value='" . $row['acc_fees'] . "'>" . $row['acc_fees'] . "</option>";
            ...
            echo "<td id='acc_remark'><input type='text' name='acc_remark[]' value='". $row['acc_remark'] ."'></td>";
...
function updateData(button) {
    ...
    var acc_fees = row.querySelector('select[name="acc_fees[]"]').value;
    var acc_bal = row.querySelector('input[name="acc_bal"]').value;
    var acc_remark = row.querySelector('input[name="acc_remark[]"]').value;
    ...
    xhr.open('POST', '../php/updateacc.php', true);
    xhr.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded');
    xhr.send('roll_no=' + encodeURIComponent(rollNo) + '&acc_fees=' + encodeURIComponent(acc_fees) + '&acc_bal=' + encodeURIComponent(acc_bal)+'&acc_remark=' + encodeURIComponent(acc_remark) + '&table_nm=' + encodeURIComponent(table_nm));
}
