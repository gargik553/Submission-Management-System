...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($result->num_rows > 0) {
        ...
        while ($row = $result->fetch_assoc()) {
            ...
            echo "<td id='stud_sec_status'><select name='stud_sec_status[]' id='stud_sec_status" . $row['stud_sec_status'] . "'>";

            // Display the fetched value from the database
           //echo "<option value='" . $row['stud_sec_status'] . "'>" . $row['stud_sec_status'] . "</option>";
            ...
            echo "<td id='stud_sec_remark'><input type='text' name='stud_sec_remark[]' value='". $row['stud_sec_remark'] ."'></td>";
...
function updateData(button) {
    ...
    var stud_sec_status = row.querySelector('select[name="stud_sec_status[]"]').value;
   
    var stud_sec_remark = row.querySelector('input[name="stud_sec_remark[]"]').value;
    ...
    xhr.open('POST', '../php/updatestu_sec.php', true);
    xhr.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded');
    xhr.send('roll_no=' + encodeURIComponent(rollNo) + '&stud_sec_status=' + encodeURIComponent(stud_sec_status)+'&stud_sec_remark=' + encodeURIComponent(stud_sec_remark) + '&table_nm=' + encodeURIComponent(table_nm));
}
