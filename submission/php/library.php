...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($result->num_rows > 0) {
        ...
        while ($row = $result->fetch_assoc()) {
            ...
            echo "<td id='library_dues'><select name='library_dues[]' id='library_dues" . $row['library_dues'] . "'>";
            ...
            echo "<td id='library_remark'><input type='text' name='library_remark[]' value='". $row['library_remark'] ."'></td>";
...
function updateData(button) {
    ...
    var library_dues = row.querySelector('select[name="library_dues[]"]').value;
   
    var library_remark = row.querySelector('input[name="library_remark[]"]').value;
    ...
    xhr.open('POST', '../php/updatelib.php', true);
    xhr.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded');
    xhr.send('roll_no=' + encodeURIComponent(rollNo) + '&library_dues=' + encodeURIComponent(library_dues)+'&library_remark=' + encodeURIComponent(library_remark) + '&table_nm=' + encodeURIComponent(table_nm));
}
